...
SELECT Name AS EmployeeName, Salary, DepartmentName FROM Employees JOIN Departments ON Employees.DepartmentID = Departments.DepartmentID WHERE Employees.Salary > 50000

-- 2. Using the Customers and Orders tables, write a query to display customer names and order dates for orders placed in the year 2023.
-- 🔁 Expected Output: FirstName, LastName, OrderDate
SELECT FirstName, LastName, OrderDate FROM Customers JOIN Orders ON Customers.CustomerID = Orders.CustomerID WHERE YEAR(OrderDate) = 2023
...
SELECT  Name AS EmployeeName, DepartmentName FROM Employees LEFT JOIN Departments ON Employees.DepartmentID = Departments.DepartmentID
...
SELECT SupplierName, ProductName FROM Suppliers LEFT JOIN Products ON Suppliers.SupplierID = Products.SupplierID
...
SELECT Orders.OrderID, Orders.OrderDate, Payments.PaymentDate, Payments.Amount FROM Orders FULL JOIN Payments ON Orders.OrderID = Payments.OrderID
...
SELECT A.Name EmployeeName, B.Name ManagerName FROM Employees A LEFT JOIN Employees B ON A.ManagerID = B.EmployeeID WHERE B.ManagerID IS NOT NULL

-- 7. Using the Students, Courses, and Enrollments tables, write a query to list the names of students who are enrolled in the course named 'Math 101'.
-- 🔁 Expected Output: StudentName, CourseName
SELECT Name StudentName, CourseName FROM Students JOIN Enrollments ON Students.StudentID = Enrollments.StudentID
...
SELECT FirstName, LastName, SUM(Quantity) AS Quantity FROM Customers JOIN Orders ON Customers.CustomerID = Orders.CustomerID GROUP BY FirstName, LastName HAVING SUM(Quantity) > 3

-- 9. Using the Employees and Departments tables, write a query to list employees working in the 'Human Resources' department.
-- 🔁 Expected Output: EmployeeName, DepartmentName
SELECT NAME EmployeeName, DepartmentName FROM Employees JOIN Departments ON Employees.DepartmentID = Departments.DepartmentID WHERE DepartmentName='Human Resources'
...
SELECT DepartmentName, COUNT(*) EmployeeCount FROM Departments JOIN Employees ON Employees.DepartmentID = Departments.DepartmentID GROUP BY DepartmentName HAVING COUNT(*) > 10
...
SELECT Products.ProductID, Products.ProductName FROM Products LEFT JOIN Sales ON Products.ProductID = Sales.ProductID WHERE Sales.ProductID IS NULL

-- 12. Using the Customers and Orders tables, write a query to return customer names who have placed at least one order.
-- 🔁 Expected Output: FirstName, LastName, TotalOrders
SELECT FirstName, LastName, COUNT(DISTINCT Quantity) TotalOrders FROM Customers JOIN Orders ON Customers.CustomerID = Orders.CustomerID GROUP BY FirstName, LastName HAVING COUNT(DISTINCT Quantity) >=1 
...
-- 🔁 Expected Output: EmployeeName, DepartmentName
SELECT Name EmployeeName, DepartmentName FROM Employees JOIN Departments ON Employees.DepartmentID = Departments.DepartmentID ORDER BY DepartmentName ASC

-- 14. Using the Employees table, write a query to find pairs of employees who report to the same manager.
-- 🔁 Expected Output: Employee1, Employee2, ManagerID
SELECT A.Name Employee1, B.Name Employee2, A.ManagerID FROM Employees A JOIN Employees B ON A.ManagerID = B.DepartmentID WHERE A.EmployeeID != B.EmployeeID

-- 15. Using the Orders and Customers tables, write a query to list all orders placed in 2022 along with the customer name.
-- 🔁 Expected Output: OrderID, OrderDate, FirstName, LastName
SELECT OrderID, OrderDate, FirstName, LastName FROM Orders JOIN Customers ON Orders.CustomerID = Customers.CustomerID WHERE YEAR(OrderDate) = 2022
...
-- 🔁 Expected Output: EmployeeName, Salary, DepartmentName
SELECT NAME EmployeeName, Salary, DepartmentName FROM Employees JOIN Departments ON Employees.DepartmentID = Departments.DepartmentID WHERE DepartmentName ='Sales' AND Salary = 60000
...
SELECT E.Name AS EmployeeName, E.Salary FROM Employees E JOIN (SELECT AVG(Salary) AS avg_salary FROM Employees) AS avg_salary_table ON E.Salary > avg_salary_table.avg_salary;
...
SELECT Orders.OrderID, OrderDate FROM Orders JOIN Payments ON Orders.OrderID = Payments.OrderID WHERE  YEAR(OrderDate) < 2020 AND Payments.OrderID IS NULL

-- 22. Using the Products and Categories tables, write a query to return products that do not have a matching category.
-- 🔁 Expected Output: ProductID, ProductName
SELECT Products.ProductID, ProductName FROM Products LEFT JOIN Categories ON Products.Category = Categories.CategoryName WHERE Categories.CategoryName IS NULL
...
SELECT A.Name Employee1, B.Name Employee2, A.ManagerID, A.Salary FROM Employees A JOIN Employees B ON A.ManagerID = B.DepartmentID WHERE A.EmployeeID != B.EmployeeID AND A.Salary > 60000 
...
-- 🔁 Expected Output: EmployeeName, DepartmentName
SELECT NAME EmployeeName, DepartmentName FROM Employees JOIN Departments ON Employees.DepartmentID = Departments.DepartmentID WHERE DepartmentName LIKE 'M%'
...
SELECT  SaleID, ProductName, SUM(SaleAmount) AS SaleAmount FROM Products JOIN Sales ON Products.ProductID = Sales.ProductID GROUP BY SaleID, ProductName HAVING SUM(SaleAmount) > 500
...
SELECT Name StudentName, CourseName FROM Students 
					LEFT JOIN Enrollments ON Students.StudentID = Enrollments.StudentID
					LEFT JOIN Courses ON Enrollments.CourseID = Courses.CourseID AND Courses.CourseName = 'Math 101'
...
SELECT Orders.OrderID, OrderDate, Payments.PaymentID FROM Orders LEFT JOIN Payments ON Orders.OrderID = Payments.OrderID WHERE Payments.PaymentID IS NULL;
...
SELECT Products.ProductID, Products.ProductName, Categories.CategoryName FROM Products JOIN Categories ON Products.Category = Categories.CategoryName WHERE Categories.CategoryName IN ('Electronics', 'Furniture');
