...
SELECT DepartmentName, Employees.Name FROM Departments CROSS JOIN Employees 
...
SELECT Suppliers.SupplierName, Products.ProductName FROM Products JOIN Suppliers ON Products.SupplierID = Suppliers.SupplierID;

-- 4. Using Orders, Customers table List customer names and their orders ID. 
SELECT CONCAT(FirstName,' ', LastName) AS Customer_Name , OrderID FROM Orders JOIN Customers ON Orders.CustomerID = Customers.CustomerID

-- 5. Using Courses, Students table Get all combinations of students and courses.
SELECT Students.Name, CourseName FROM Courses CROSS JOIN Students 

-- 6. Using Products, Orders table Get product names and orders where product IDs match.
SELECT ProductName, OrderID FROM Products JOIN Orders ON Products.ProductID = Orders.ProductID

-- 7. Using Departments, Employees table List employees whose DepartmentID matches the department.
SELECT DepartmentName, Name FROM Departments JOIN Employees ON Departments.DepartmentID = Employees.DepartmentID

-- 8. Using Students, Enrollments table List student names and their enrolled course IDs.
SELECT Name, CourseID FROM Students JOIN Enrollments ON Students.StudentID = Enrollments.StudentID

-- 9. Using Payments, Orders table List all orders that have matching payments.
SELECT Orders.OrderID, Orders.OrderDate, Payments.PaymentMethod, Payments.Amount FROM Payments JOIN Orders ON Payments.OrderID = Orders.OrderID

-- 10. Using Orders, Products table Show orders where product price is more than 100.
SELECT Orders.OrderID, Orders.OrderDate, Products.ProductName, Products.Price FROM Orders JOIN Products ON Products.ProductID = Orders.ProductID WHERE Price > 100
...
SELECT DepartmentName, Name FROM Employees JOIN Departments ON Employees.DepartmentID != Departments.DepartmentID

-- 12. Using Orders, Products table Show orders where ordered quantity is greater than stock quantity.
SELECT Orders.OrderID, Orders.OrderDate, Orders.Quantity, Products.ProductName, Products.StockQuantity FROM ORDERS JOIN Products ON Products.ProductID  = Orders.ProductID WHERE Orders.Quantity > Products.StockQuantity;

-- 13. Using Customers, Sales table List customer names and product IDs where sale amount is 500 or more.
SELECT CONCAT(FirstName, ' ', LastName) AS customer_names, ProductID FROM Customers JOIN Sales ON Customers.CustomerID = Sales.CustomerID WHERE SaleAmount > 500

-- 14. Using Courses, Enrollments, Students table List student names and course names they’re enrolled in.
SELECT Name, CourseName FROM Students JOIN Enrollments ON Students.StudentID = Enrollments.StudentID JOIN Courses ON Enrollments.CourseID = Courses.CourseID

-- 15. Using Products, Suppliers table List product and supplier names where supplier name contains “Tech”.
SELECT ProductName, SupplierName FROM Products JOIN Suppliers ON Products.SupplierID = Suppliers.SupplierID WHERE SupplierName LIKE 'Tech%'

-- 16. Using Orders, Payments table Show orders where payment amount is less than total amount.
SELECT Orders.OrderID, Orders.TotalAmount, Payments.Amount FROM Orders JOIN Payments ON Orders.OrderID=  Payments.OrderID WHERE TotalAmount > Amount

-- 17. Using Employees table List employee names with salaries greater than their manager’s salary.
SELECT e1.Name AS EmployeeName, e1.Salary AS EmployeeSalary, e2.Name AS ManagerName, e2.Salary AS ManagerSalary FROM Employees e1 JOIN Employees e2 ON e1.ManagerID = e2.EmployeeID WHERE e1.Salary > e2.Salary;

-- 18. Using Products, Categories table Show products where category is either 'Electronics' or 'Furniture'.
SELECT ProductName, CategoryName FROM Products JOIN Categories ON Products.Category = Categories.CategoryID WHERE CategoryName IN( 'Electronics' , 'Furniture')

-- 19. Using Sales, Customers table Show all sales from customers who are from 'USA'.
SELECT CONCAT(FirstName, ' ', LastName) AS Customers_Name, SUM(SaleAmount) AS Total, Country FROM Sales JOIN Customers ON Sales.CustomerID = Customers.CustomerID WHERE Country = 'USA' GROUP BY Customers.CustomerID, FirstName, LastName, Country

-- 20. Using Orders, Customers table List orders made by customers from 'Germany' and order total > 100.
SELECT CONCAT(FirstName, ' ', LastName) AS Customers_Name, SUM(TotalAmount) AS Total, Country FROM Orders JOIN Customers ON Orders.CustomerID = Customers.CustomerID WHERE Country = 'Germany' GROUP BY Customers.CustomerID, FirstName, LastName, Country HAVING SUM(TotalAmount) > 100

-- 🔴 Hard (5 puzzles)

-- 21. Using Employees table List all pairs of employees from different departments. 
SELECT e1.Name AS Employee1, e1.DepartmentID AS Dept1, 
       e2.Name AS Employee2, e2.DepartmentID AS Dept2
FROM Employees e1
...
SELECT PaymentMethod, ProductName, Amount, (Quantity * Price) AS paid_amount FROM Payments
...
SELECT e1.EmployeeID AS ManagerID, e1.Name AS ManagerName, e1.Salary AS ManagerSalary,
       e2.EmployeeID AS EmployeeID, e2.Name AS EmployeeName, e2.Salary AS EmployeeSalary
FROM Employees e1
...
SELECT Customers.CustomerID, CONCAT(Customers.FirstName, Customers.LastName) AS CUS_NAME
FROM Customers
