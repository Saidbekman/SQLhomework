...
SELECT Category, SUM(StockQuantity) AS TOTAL_Category FROM Products GROUP BY Category

-- 2. Using Products table, get the average price of products in the 'Electronics' category.
SELECT Category, AVG(StockQuantity) AS AVG_Category FROM Products WHERE Category ='Electronics' GROUP BY Category 

-- 3. Using Customers table, list all customers from cities that start with 'L'.
--		FIRST FORM
SELECT City, FirstName, LastName  FROM Customers WHERE City LIKE 'L%'
--		SECOND FORM
...
--		SECOND FORM
SELECT TOP 1 ProductName, MAX(Price) AS PRICE FROM Products GROUP BY ProductName ORDER BY Price DESC

-- 7. Using Products table, use IIF to label stock as 'Low Stock' if quantity < 30, else 'Sufficient'.
SELECT Category, ProductName, IIF(StockQuantity < 30, 'Low Stock', 'Sufficient') AS quantity_status FROM Products

-- 8. Using Customers table, find the total number of customers in each country.
SELECT Country, COUNT(CustomerID) AS TOTAL_Customer FROM Customers GROUP BY Country
...
SELECT DISTINCT CustomerID FROM Orders WHERE YEAR(OrderDate)=2023
EXCEPT 
...
SELECT YEAR(OrderDate), AVG(TotalAmount) AS average_order_amount  FROM Orders GROUP BY YEAR(OrderDate)
...
SELECT ProductName, Price, 
		CASE WHEN Price <100 THEN 'Low' WHEN Price <500 THEN 'Mid' ELSE 'High' END AS GROUP_PRICE
FROM Products

-- 15. Using Customers table, list all unique cities where customers live, sorted alphabetically.
SELECT City, FirstName, LastName FROM Customers ORDER BY City ASC
SELECT DISTINCT City FROM Customers ORDER BY City ASC

-- 16. Using Sales table, find total sales per product Id.
SELECT ProductID, SUM(SaleAmount) AS SaleAmount FROM Sales GROUP BY ProductID
...
SELECT TOP 3 CustomerID, SUM(TotalAmount) AS TOTAL_SUM FROM Orders GROUP BY CustomerID ORDER BY TOTAL_SUM DESC
...
EXCEPT 
SELECT DISTINCT ProductID, ProductName FROM Products_Discounted 

-- 21. Using Products and Sales tables, list product names and the number of times each has been sold. (Research for Joins)
SELECT ProductName, COUNT(Sales.ProductID) AS SOLD FROM Products
JOIN Sales ON Products.ProductID = Sales.ProductID
GROUP BY ProductName ORDER BY SOLD

-- 22. Using Orders table, find top 5 products (by ProductID) with the highest order quantities.
SELECT TOP 5 ProductID, SUM(Quantity) AS order_quantities FROM Orders GROUP BY ProductID ORDER BY order_quantities DESC
