...
Select *, 
	IIF( StockQuantity > 100, 'Yes', 'No') as CT_Stock
from Products_Discounted
order by CT_Stock desc 
...
Select *, 
	IIF( Price > 100, 'Expensive', 'Affordable') as CT_Price
from Products_Discounted
order by CT_Price desc 
...
-- 17. Write a query that uses a CASE statement to determine the discount percentage based on the quantity purchased. Use orders table. Result set should show customerid, quantity and discount percentage. The discount should be applied as follows: More than 1 item: 3% Between 1 and 3 items : 5% Otherwise: 7%
