...
--		First form 
SELECT Category, MIN(Price) AS MIN_MAX FROM Products GROUP BY Category;
SELECT Category, MAX(Price) AS MIN_MAX FROM Products GROUP BY Category;
--		Second form
...
SELECT DepartmentName, COUNT(EmployeeID) FROM Employees GROUP BY DepartmentName HAVING COUNT(EmployeeID) > 5

-- 🟠 Medium-Level Tasks (9)
-- 11. Write a query to calculate the total sales and average sales for each product category from the Sales table.
SELECT ProductID, SUM(SaleAmount) AS TOTAL_SALES, AVG(SaleAmount) AS AVG_SALES FROM Sales GROUP BY ProductID;

-- 12. Write a query that uses COUNT(columnname) to count the number of employees from the Department HR.
SELECT COUNT(EmployeeID) AS HR_Employee FROM Employees WHERE DepartmentName = 'HR';
...
SELECT DepartmentName, AVG(Salary) AS average_salary FROM Employees GROUP BY DepartmentName;
...
SELECT DepartmentName, AVG(Salary) AS average_salary, COUNT(*) AS employees_working  FROM Employees GROUP BY DepartmentName;
...
SELECT YEAR(SaleDate) AS SALE_YEAR, SUM(SaleAmount) AS total_sales_for_each_year FROM Sales GROUP BY YEAR(SaleDate);

-- 18. Write a query that uses COUNT to show the number of customers who placed at least 3 orders.
SELECT CustomerID, COUNT(OrderID) AS Total FROM Orders GROUP BY CustomerID HAVING COUNT(OrderID) >= 3;
...
SELECT DepartmentName, SUM(Salary) AS SUM_SALARY FROM Employees GROUP BY DepartmentName HAVING SUM(Salary) > 500000
...
SELECT ProductID, AVG(TotalAmount) AS AVG_TOTAL FROM Orders GROUP BY ProductID HAVING AVG(TotalAmount) > 200
...
SELECT CustomerID, SUM(SaleAmount) AS SUM_SALARY FROM Sales GROUP BY CustomerID HAVING SUM(SaleAmount) > 1500
...
SELECT DepartmentName, SUM(Salary) AS SUM_SALARY, AVG(Salary) AS AVG_SALARY FROM Employees GROUP BY DepartmentName HAVING  AVG(Salary) > 65000
...
SELECT CustomerID, MAX(TotalAmount) AS Max_TotalAmount, MIN(TotalAmount) AS Min_TotalAmount FROM Orders GROUP BY CustomerID HAVING MIN(TotalAmount) >= 50;
...
SELECT YEAR(OrderDate) AS Order_Year, MONTH(OrderDate) AS Order_Month, SUM(TotalAmount) AS Total_Sales, COUNT(DISTINCT ProductID) AS Unique_Products_Sold FROM Orders GROUP BY YEAR(OrderDate), MONTH(OrderDate) HAVING COUNT(DISTINCT ProductID) > 8;

-- 25. Write a query to find the MIN and MAX order quantity per Year. From orders table. (Do some research)
SELECT YEAR(OrderDate) AS Order_Year, MIN(Quantity) AS Min_Order_Quantity, MAX(Quantity) AS Max_Order_Quantity FROM Orders GROUP BY YEAR(OrderDate) ORDER BY Order_Year;
