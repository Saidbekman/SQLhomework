...
Select top 5 * from Employees Where FirstName is not null
...
SELECT EmployeeID, FirstName, ISNULL(Email, 'noemail@example.com') AS Email FROM Employees;
...
Select * from Employees
Where Age >30 and Age <40 and DepartmentName = 'Marketing'
...
Select * from Products_Discounted where Price<= 1000 and StockQuantity > 50 order by StockQuantity ASC
...
Select  ISNULL (FirstName, LastName), DepartmentName from Employees
...
Select top 10 * from Products_Discounted order by StockQuantity DESC
...
CREATE TABLE Orders (
    ...
    FOREIGN KEY (ProductID) REFERENCES Products_Discounted(ProductID)
);
